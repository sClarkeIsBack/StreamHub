...
class Database(object):
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[script.streamhub.tvguide] Database is locked, bailing out...', xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[script.streamhub.tvguide] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[script.streamhub.tvguide] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        c = self.conn.cursor()
        try:
            xbmc.log('[script.streamhub.tvguide] Updating caches...', xbmc.LOGDEBUG)
...
class XMLTVSource(Source):
    ...
    def getDataFromExternal(self, date, progress_callback=None):
            u = urllib2.urlopen('https://raw.githubusercontent.com/sClarkeIsBack/StreamHub/master/guide.xml', timeout=30)
            xml = u.read()
            u.close()

...
def install(name,url):
	...
	if xbmc.getCondVisibility('system.platform.android'):
		...
		dp.create('MediaHubIPTV',"","",'Downloading:' + name)
		lib=os.path.join(path, 'app.apk')
		downloader.download(url, lib, dp)
		xbmcgui.Dialog().ok('MediaHubIPTV', "[COLOR white]Launching the installer[/COLOR]" , "[COLOR white]You will now be asked to install[/COLOR] [B][COLOR lime]%s[/COLOR][/B]"%name)

...
from resources.modules import metacache
from resources.modules import client
from resources.modules import control
from resources.modules import regex
from resources.modules import trailer
from resources.modules import workers
from resources.modules import youtube
from resources.modules import views
...
class indexer:
    ...
    def root(self):
        try:
            regex.clear()
            url = 'https://raw.githubusercontent.com/sClarkeIsBack/StreamHub/master/Links/NewHome.xml'
...
if action == None:
    ...
    indexer().youtube(url, action)

elif action == 'play':
    
    player().play(url, content)

elif action == 'browser':
    
    resolver().browser(url)

elif action == 'search':
    
    indexer().search(url=None)

elif action == 'addSearch':
    
    indexer().addSearch(url)

...
def freetvip():
	...
	part  = regex_from_to(open,'property="og:description" content="','/>')
	log(part)
	all  = re.compile('http://(.+?)m3u ',re.MULTILINE|re.DOTALL).findall(part)
	for url in all:
		url = 'http://'+url+'m3u'
		url = (url).replace('amp;','')
		addDir(url,urllib.quote_plus(url),3,fanart,'','')

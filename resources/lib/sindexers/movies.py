...
from resources.lib.smodules import metacache
    ...
    def movieDirectory(self, items):
        ...
        for i in items:
            try:
                ...
                url = '%s?action=playscrape&title=%s&year=%s&imdb=%s&meta=%s&t=%s' % (sysaddon, systitle, year, imdb, sysmeta, self.systime)
                sysurl = urllib.quote_plus(url)

                path = '%s?action=playscrape&title=%s&year=%s&imdb=%s' % (sysaddon, systitle, year, imdb)
